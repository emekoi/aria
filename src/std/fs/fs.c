...
static PathNode *newNode(const char *path) {
  int res;
  int len = strlen(path);
  PathNode *p = FS_CALLOC(1, sizeof(*p) + len + 1);
  if (!p) return NULL;
  strcpy(p->path, path);
...
static int fileInfo(
  const char *filename, unsigned *mtime, size_t *size, int *isdir, int *isreg
) {
...
int fs_exists(const char *filename) {
  return fileInfo(filename, NULL, NULL, NULL, NULL) == FS_ESUCCESS;
}


int fs_modified(const char *filename, unsigned *mtime) {
  return fileInfo(filename, mtime, NULL, NULL, NULL);
}


int fs_size(const char *filename, size_t *size) {
  return fileInfo(filename, NULL, size, NULL, NULL);
}
...
int fs_isDir(const char *filename) {
  int res;
  int err = fileInfo(filename, NULL, NULL, &res, NULL);
  if (err) return 0;
  return res;
}
...
int fs_isFile(const char *filename) {
  int res;
  int err = fileInfo(filename, NULL, NULL, NULL, &res);
  if (err) return 0;
  return res;
}
...
static fs_FileListNode *newFileListNode(const char *path) {
  fs_FileListNode *n = FS_MALLOC(sizeof(*n) + strlen(path) + 1);
  if (!n) return NULL;
  n->next = NULL;
  n->name = (void*) (n + 1);
  strcpy(n->name, path);
